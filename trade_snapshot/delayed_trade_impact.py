...
class PreparedDelayedBaseline:
    ...
    _conditioned_before_cache: OrderedDict[
    ...
    def __init__(self, baseline: PreparedSeasonBaseline) -> None:
        ...
        scenarios = tuple(baseline.iter_baseline_scenarios())
        _validate_before_scenarios(baseline, scenarios)
        object.__setattr__(self, "baseline", baseline)
        object.__setattr__(self, "before_scenarios", scenarios)
        object.__setattr__(self, "_conditioned_before_cache", OrderedDict())
    ...
    def _conditioned_before(
        ...
        while (
            len(self._conditioned_before_cache)
            > _MAX_CONDITIONED_BEFORE_CACHE_SIZE
        ):
            self._conditioned_before_cache.popitem(last=False)
        return result
...
class PreparedDelayedRosterChange:
    ...
    def __init__(
        ...
        after = baseline.scenarios.with_rosters(after_rows)
