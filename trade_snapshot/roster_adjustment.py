...
class PreparedRosterAdjuster:
    ...
    def adjust_teams(
        ...
        source_by_player = {
            player_id: roster
            for roster, outgoing, _ in normalized
        ...
        for roster, outgoing, incoming in sorted(
            ...
            adjusted = self._balance(
                roster,
                raw_players,
                reserve_candidates_by_player=_multi_team_reserve_candidates(
                    roster,
                    outgoing_set,
                    incoming,
                    source_by_player,
...
def _transferred_reserve_candidates(
    ...
    return _multi_team_reserve_candidates(
        destination,
        outgoing_player_ids,
        incoming_player_ids,
        {player_id: source for player_id in incoming_player_ids},
    )


def _multi_team_reserve_candidates(
    destination,
    outgoing_player_ids,
    incoming_player_ids,
    source_by_player,
    ...
    result.update(
        {
            player_id: source_by_player[player_id].reserve_slot_by_player[player_id]
            for player_id in incoming_player_ids
            if player_id in source_by_player[player_id].reserve_slot_by_player
        }
    )
    return result
