...
from .draft_availability import RosterAvailabilityReport
...
HISTORICAL_CORPUS_SCHEMA_VERSION = 2
...
class HistoricalSeason:
    ...
    availability_reports: tuple[RosterAvailabilityReport, ...] = ()
    ...
    def __post_init__(self) -> None:
        ...
        reports = tuple(self.availability_reports)
        if any(not isinstance(row, RosterAvailabilityReport) for row in reports):
            raise ValueError("availability_reports must contain RosterAvailabilityReport values")
        if any(row.status.inferred for row in reports):
            raise ValueError("inferred absence is generated by league rules, not historical source reports")
        if any(row.player_id not in ids or row.week not in expected for row in reports):
            raise ValueError("availability report must refer to a player and week in this season")
        if len({(row.player_id, row.week) for row in reports}) != len(reports):
            raise ValueError("availability reports cannot contain duplicate player weeks")
        object.__setattr__(self, "availability_reports", tuple(sorted(
            reports, key=lambda row: (row.week, row.player_id),
        )))
    ...
    def to_record(self) -> dict[str, object]:
        ...
        if self.availability_reports:
            record["availability_reports"] = [row.to_record() for row in self.availability_reports]
        return record
...
class HistoricalCorpus:
    ...
    def to_record(self) -> dict[str, object]:
        return {
            ...
            "schema_version": 2 if any(row.availability_reports for row in self.seasons) else 1,
    ...
    def from_record(cls, record: Mapping[str, object]) -> "HistoricalCorpus":
        ...
        if (
            ...
            or record["schema_version"] not in {1, HISTORICAL_CORPUS_SCHEMA_VERSION}
        ...
        if corpus.to_record()["schema_version"] != record["schema_version"]:
            raise ValueError("historical corpus schema version is not canonical")
    ...
    def summary(self) -> dict[str, object]:
        return {
            ...
            "availability_report_count": sum(len(row.availability_reports) for row in self.seasons),
...
def _season_from_record(value: object) -> HistoricalSeason:
    keys = {"season", "preseason_as_of", "season_kickoff_at", "available_weeks", "players"}
    if isinstance(value, Mapping) and "availability_reports" in value:
        keys.add("availability_reports")
    row = _exact_object(
        "historical season",
        value,
        keys,
    ...
    return HistoricalSeason(
        ...
        availability_reports=tuple(
            RosterAvailabilityReport.from_record(item)
            for item in _array("availability_reports", row.get("availability_reports", []))
        ),
    )
