...
class WeeklyCollectionJobs:
    ...
    def _run(self, job: _CollectionJob) -> None:
        try:
            ...
            if publication is None:
                ...
                self._save_history_attempt(bundle.bundle_id, history_attempt)
            else:
                history_attempt = self._publish_bound_publication(publication)
            self._update(
                job,
                status="complete",
    ...
    def _publish_bound_publication(
        ...
        self._save_history_attempt(publication.bundle.bundle_id, attempt)
    ...
    def _save_history_attempt(
        self, bundle_id: str, attempt: WeeklyHistoryAttempt
    ) -> None:
        """Persist a small credential-free diagnostic without gating publication."""

        directory = self._data_directory / _HISTORY_ATTEMPT_DIRECTORY
