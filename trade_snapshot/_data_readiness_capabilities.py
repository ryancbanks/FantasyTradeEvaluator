...
def _capability_decisions(
    ...
    *,
    ...
    state = bundle.state
    independent_power = bundle.methodology_mode == "independent"
    ...
    formula_identity_matches = None
    required_features = set()
    if not independent_power:
        ...
        required_features.update(bundle.strength_formula.residual_weights)
    ...
    required_current_providers = frozenset()
    required_full_ros_providers = frozenset()
    required_projection_evidence_complete = True
    formula_uses_projection = False
    if not independent_power:
        availability = projection_availability_requirements(
            required_features,
            tuple(row.provider for row in bundle.ensemble_config.provider_weights),
        )
        required_current_providers = availability.current_providers
        required_full_ros_providers = availability.full_ros_providers
    ...
    projection_schedule_binding_complete = _projection_schedule_binding_complete(
        bundle
    )
    schedule_inputs_ready = schedule_complete or (
        independent_power and projection_schedule_binding_complete
    )
    ...
    projection_rows_scoring_bound = all(
    ...
    projection_scoring_bound = (
        projection_rows_scoring_bound
        and (
            independent_power
            or bundle.projection_source_manifest.evaluation_scoring_profile_id
            == bundle.scoring_profile.scoring_profile_id
        )
    )
    independent_policy_identity_matches = (
        independent_power
        and bundle.independent_power_disclosure.weekly_snapshot_id
        == state.snapshot_id
        and bundle.independent_power_disclosure.strength_model_id
        == bundle.strength_model.model_id
        and set(bundle.independent_power_disclosure.provider_names)
        == {
    ...
    if independent_power:
        power_missing = _missing_labels(
            (
                independent_policy_identity_matches,
                "independent power disclosure bound to the retained strength model",
            ),
            (
                roster_identity_complete,
                "complete roster, eligibility, and strength identities",
            ),
            (
                projection_grid_complete,
                "complete remaining regular-season projection grid",
            ),
            (
                projection_scoring_bound,
                "projection evidence bound to the selected scoring profile",
            ),
        )
    else:
        power_missing = _missing_labels(
    ...
    expected_standings_missing = _missing_labels(
        ...
        (schedule_inputs_ready, "complete NFL schedule and player-team binding"),
    ...
    lab_missing = _missing_labels(
        ...
        (
            independent_power or ecr_pair_complete,
            "weekly and rest-of-season ECR evidence",
        ),
    ...
    benchmark_complete = (
        bundle.fantasypros_benchmark is not None
        and {row.team_id for row in bundle.fantasypros_benchmark.teams} == team_ids
    ...
    common_evidence = {
        ...
        "nfl_schedule_artifact_retained": bundle.nfl_schedule is not None,
        "projection_schedule_binding_complete": (
            projection_schedule_binding_complete
        ),
    }
    if independent_power:
        power_limitations = [bundle.independent_power_disclosure.notice]
    elif holdout_validated_power:
        power_limitations = (
        [
        ...
        power_limitations = ["The current power formula is a disclosed local surrogate."]
    ...
    return {
        "fantasypros_style_power": {
            ...
            "uses": [
                ...
                "eligibilities",
                *(
                    ["independent_power_policy", "remaining_projection_ensemble"]
                    if independent_power
                    else []
                ),
                *(
            ...
            "evidence": {
                **common_evidence,
                ...
                "required_current_providers": sorted(required_current_providers),
                "required_full_ros_providers": sorted(required_full_ros_providers),
        ...
        "player_lab": {
            ...
            "limitations": [
                *(
                ...
                _AVAILABILITY_LIMITATION,
                *([] if independent_power else ecr_coverage_limitations),
                *(
                    [
                        limitation
        ...
        "fantasypros_comparison_benchmark": {
            ...
            "limitations": (
                [
                    "Independent bundles neither retain nor use a FantasyPros "
            ...
            "missing": (
                []
                if independent_power or benchmark_complete
                else ["benchmark coverage for every league team"]
            ),
        },
...
def _projection_schedule_binding_complete(bundle):
    """Verify the player-week game context retained by the calculation grid."""

    bindings = {}
    for row in bundle.projections:
        if row.nfl_team_id is None:
            return False
        context = (
            ("bye",)
        ...
        if row.status is ProjectionStatus.OBSERVED and any(
            value is None for value in context[1:]
        ):
            return False
        key = row.nfl_team_id, row.week
        known = bindings.setdefault(key, context)
        if known != context:
            return False
