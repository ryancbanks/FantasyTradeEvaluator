...
class DataReadinessSnapshot:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or self.fantasypros_comparison_team_count < 0
        ):
            raise ValueError(
                "fantasypros_comparison_team_count must be non-negative"
            )
        source_counts = (
        ...
        if bool(formats) != bool(self.projection_source_count):
            raise ValueError(
                "projection source scoring formats must match source coverage"
            )
        object.__setattr__(
            self,
