(() => {
  ...
  const readJson = async (expectedUrl, maximumBytes, deadline) => {
  ...
  handlers["espn.authenticated_json"] = async (options) => {
    ...
    const views = ["mTeam", "mRoster", "mSettings", "mMatchup", "mStandings"];
    ...
    const league = await readJson(leagueUrl, options.maximum_bytes, deadline);
