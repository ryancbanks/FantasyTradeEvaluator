...
def _evaluate_pair(bundle, left, right, left_candidates, right_candidates, status):
    ...
    for left_index, left_player_id in enumerate(left_candidates):
        for right_index, right_player_id in enumerate(right_candidates):
            result = prepared.evaluate(
                TradeCandidate((left_player_id,), (right_player_id,)),
                candidate_index=left_index * len(right_candidates) + right_index,
            )
