...
window.ProgressUi = (() => {
  ...
  function writeDeviceValue(key, value) {
    ...
    } catch (_) { /* Remembering local UI preferences is optional. */ }
  }
  ...
  function setBar(track, bar, fraction, label) {
    ...
    track.setAttribute("aria-valuetext", label || (exact ? `${(fraction * 100).toFixed(0)}%` : "Working"));
  ...
  async function run(key, label, task) {
    ...
    const render = () => {
      ...
      // These one-shot tasks do not expose work-unit counts. Keep the bar
      // indeterminate instead of presenting elapsed time as completed work.
