...
window.LeagueUi = (() => {
  ...
  function selectedId() { return $("leagueSelect").value; }
  function selectedProfile() {
    const id = selectedId();
    return profiles.find(profile => profile.profile_id === id) || null;
  }
  function isUnassigned() { return selectedId() === UNASSIGNED; }
  function canCollect() {
    const profile = selectedProfile();
    return Boolean(
      profile && !profile.archived && profile.espn_league_id && profile.yahoo_league_id
    );
  }
  ...
  function canonicalYahooUrl(profile) {
    ...
    return `https://football.fantasysports.yahoo.com/${profile.season}/f1/${profile.yahoo_league_id}/players`;
  }
  ...
  async function loadEveryProfile(includeArchived) {
    ...
    do {
      ...
      const page = await api(`/api/leagues?${query}`);
      if (!Array.isArray(page.profiles)) throw new Error("The saved league list is invalid.");
      ...
      if (page.unassigned_bundle_count !== undefined) {
        ...
        if (!Number.isSafeInteger(loadedUnassignedBundleCount) || loadedUnassignedBundleCount < 0) {
  ...
  async function refresh(preferredId = null, {notify = true} = {}) {
    ...
    select.replaceChildren(new Option(profiles.length || unassignedBundleCount ? "Choose a league" : "Add your first league", ""));
    for (const profile of profiles) {
      const suffix = profile.archived ? " · archived" : "";
      select.add(new Option(`${profile.name} · ${profile.season}${suffix}`, profile.profile_id));
    }
    if (unassignedBundleCount) {
      select.add(new Option(`Unassigned imports · ${unassignedBundleCount}`, UNASSIGNED));
    }
    const remembered = ProgressUi.readDeviceValue(ACTIVE_KEY);
    const choices = new Set([...select.options].map(option => option.value));
    const target = [preferredId, previous, remembered, profiles.find(row => !row.archived)?.profile_id, unassignedBundleCount ? UNASSIGNED : null]
  ...
  function openEditor(profile = null) {
    ...
    $("leagueEditorTitle").textContent = profile ? `Edit ${profile.name}` : "Add a league";
  ...
  async function saveEditor() {
    const endpoint = editingProfileId ? `/api/leagues/${editingProfileId}` : "/api/leagues";
    $("saveLeagueButton").disabled = true;
    $("cancelLeagueEditButton").disabled = true;
    try {
      const profile = await ProgressUi.run(
  ...
  async function toggleArchive() {
    ...
    try {
      await ProgressUi.run(
        ...
        async () => {
          await api(`/api/leagues/${profile.profile_id}/${action}`, {method: "POST", body: ""});
  ...
  async function saveMyTeam(teamId, bundleId) {
    const profile = selectedProfile();
    if (!profile || typeof teamId !== "string" || !teamId || typeof bundleId !== "string" || !bundleId) return;
    const updated = await api(`/api/leagues/${profile.profile_id}/team`, {
      method: "POST",
  ...
  function importPath() {
    const profile = selectedProfile();
    return profile ? `/api/leagues/${profile.profile_id}/bundles/import` : "/api/bundles/import";
  }

  function collectionPayload() {
    const profile = selectedProfile();
    if (!profile || profile.archived) throw new Error("Choose an active league before collecting weekly data.");
    return {
  ...
  function renderAssignmentChoices() {
    ...
    for (const profile of active) select.add(new Option(`${profile.name} · ${profile.season}`, profile.profile_id));
  ...
  async function assignBundle(bundleId) {
    ...
    if (!profileId) throw new Error("Choose the league that owns this weekly bundle.");
    await ProgressUi.run(
      "bundle-assign",
      "Assigning weekly data",
      async () => {
        await api(`/api/leagues/${profileId}/bundles/${bundleId}/assign`, {method: "POST", body: ""});
  ...
  function setBusy(busy) {
    if (busy) closeEditor();
    for (const id of [
      "leagueSelect", "addLeagueButton", "editLeagueButton", "archiveLeagueButton",
      "showArchivedLeagues", "assignLeagueSelect", "assignBundleButton"
    ]) {
      const element = $(id);
  ...
  function bind(options) {
    ...
    $("leagueSelect").addEventListener("change", () => changeSelection().catch(onError));
    $("showArchivedLeagues").addEventListener("change", () => refresh(null).catch(onError));
    $("addLeagueButton").addEventListener("click", () => openEditor());
    $("editLeagueButton").addEventListener("click", () => openEditor(selectedProfile()));
    $("archiveLeagueButton").addEventListener("click", () => toggleArchive().catch(onError));
