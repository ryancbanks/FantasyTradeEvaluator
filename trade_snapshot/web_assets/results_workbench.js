...
window.ResultsWorkbench = (() => {
  ...
  function threeTeamMetrics(row, index, primaryTeamId) {
    ...
    for (const [impactIndex, impact] of row.team_impacts.entries()) {
      ...
      if (teamIds.has(teamId)) throw new TypeError(`rows[${index}].team_impacts has a duplicate team.`);
      teamIds.add(teamId);
    ...
    for (const [transferIndex, transfer] of row.transfers.entries()) {
      ...
      for (const [playerIndex, player] of transfer.players.entries()) {
        ...
        const playerId = requireText(player.player_id, `${name}.players[${playerIndex}].player_id`);
  ...
  function passesFilters(rowMetrics, controls) {
    if (controls.onlyAllParticipantsImprove &&
        !rowMetrics.playoffGains.every(value => value > 0)) return false;
  ...
  function filterAndSort(rows, context, controlValues = {}) {
    ...
    for (const key of Object.keys(context)) {
      if (!allowedContextKeys.has(key)) throw new TypeError(`Unknown context value: ${key}.`);
    }
