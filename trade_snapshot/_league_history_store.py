...
class LeagueHistoryStore:
    ...
    def ingest_snapshot(self, snapshot: LeagueHistorySnapshot) -> str:
        """Atomically copy one bundle-bounded history snapshot into this store."""

        if not isinstance(snapshot, LeagueHistorySnapshot):
            raise ValueError("snapshot must be a LeagueHistorySnapshot")
        try:
            with closing(self._connection()) as connection, connection:
                self._require_schema(connection)
                for capture in snapshot.captures:
                    self._insert_capture(connection, capture)
                for binding in snapshot.bundle_bindings:
                    self._insert_binding(connection, binding)
            return snapshot.history_revision
        except LeagueHistoryStoreError:
            raise
        except sqlite3.IntegrityError as error:
            raise LeagueHistoryConflictError(
                "league history snapshot conflicts with immutable stored data: "
                f"{error}"
            ) from None
        except (OSError, sqlite3.Error, TypeError, ValueError) as error:
            raise LeagueHistoryStoreError(
                f"could not append league history snapshot: {error}"
            ) from None
