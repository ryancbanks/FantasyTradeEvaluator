...
class LeagueWorkspaceService:
    ...
    def bundle_rows(self, profile_id: str) -> tuple[dict[str, object], ...]:
        if profile_id == UNASSIGNED_PROFILE_ID:
            ...
            for bundle_id in self.unassigned_bundle_ids():
                ...
                try:
                    rows.append(_lean_bundle(bundle_summary(load_engine_bundle(path))))
    ...
    def import_bundle(
        ...
        *,
        ...
        save_engine_bundle(bundle, self.bundle_directory / f"{bundle.bundle_id}.json")
        if profile_id is not None:
    ...
    def collection_plan(
        ...
        if not isinstance(payload, Mapping) or set(payload) != _COLLECTION_FIELDS:
        ...
        if profile.espn_collection_url is None or profile.yahoo_collection_url is None:
            raise ValueError("this league needs both ESPN and Yahoo connections")
        request = WeeklyCollectionRequest(
...
__all__ = ("LeagueWorkspaceService", "UNASSIGNED_PROFILE_ID")
