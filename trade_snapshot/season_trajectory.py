...
def build_season_trajectory(
    ...
    *,
    ...
    trajectory, _ = _build_season_trajectory(
        state,
        scenarios,
...
def build_season_trajectory_analysis(
    state: LeagueState,
    scenarios: Iterable[ScoreScenario],
    *,
    score_decimal_places: int = 2,
    ...
    """Build the trajectory and its loss/downturn index in one scenario pass."""

    trajectory, trigger_index = _build_season_trajectory(
        state,
        scenarios,
        score_decimal_places=score_decimal_places,
        random_seed=random_seed,
    ...
    if trigger_index is None:  # Internal invariant kept explicit for type checkers.
        raise AssertionError("combined trajectory analysis omitted its trigger index")
    return trajectory, trigger_index


def _build_season_trajectory(
    state,
    scenarios,
    *,
    ...
    trigger_keys = (
        tuple((team_id, week) for team_id in team_ids for week in weeks)
        if collect_triggers
        else ()
    )
    trigger_matches = (
        {key: [] for key in trigger_keys} if collect_triggers else None
    )
    trigger_eligible = (
...
def build_loss_and_downward_scenario_index(
    ...
    *,
    ...
    return _trigger_index(keys, matched, eligible, total)
    ...
    for team in state.teams:
        ...
        for path in _team_future_path(
            ...
            if slope is not None:
                ...
                if trigger_matches is not None:
                    trigger_key = team_id, week
                    trigger_eligible[trigger_key] += 1
                    if outcome == "loss" and downward:
                        trigger_matches[trigger_key].append(scenario_index)
...
def _projected_record(team_id, week, names, state, aggregate, count):
    ...
    ordered_win_percentages = sorted(aggregate["win_percentages"])
    ordered_slopes = sorted(aggregate["slopes"])
    win_conditional = (
    ...
    return {
        ...
        "median_cumulative_win_percentage": _ordered_quantile(
            ordered_win_percentages, 0.50
        ),
        "cumulative_win_percentage_p10": _ordered_quantile(
            ordered_win_percentages, 0.10
        ),
        "cumulative_win_percentage_p90": _ordered_quantile(
            ordered_win_percentages, 0.90
        ),
        "median_record_slope": _ordered_quantile(ordered_slopes, 0.50),
        "record_slope_p10": _ordered_quantile(ordered_slopes, 0.10),
        "record_slope_p90": _ordered_quantile(ordered_slopes, 0.90),
...
def _ordered_quantile(ordered, probability):
    if not ordered:
        return None
...
def _trigger_index(keys, matched, eligible, total):
    return {
