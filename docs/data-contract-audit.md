...
| Player Lab and current-player projection coverage | Weekly fused values; full-NFL-ROS values; individual provider rows/raw stats; direct/derived origin; status observations; ECR; ownership/eligibility; waiver provenance; independently captured active-player identities | Pure local read of v8, its retained full-catalog projection snapshot, and the weekly Sleeper active-player catalog | Player Lab schema v5 reconciles displayed cells to bundle evidence. Data-readiness schema v5 separately reports matched, projected, missing, and unmatched identities by declared position, provider, and current-week/remaining-season horizon, plus roster-specific gaps | **Implemented with limitations.** A captured catalog row never counts as a usable projection. Completeness is claimed only when every identity in the declared fantasy-position scope reconciles and every configured provider covers every horizon; unresolved identities, unpublished values, and active positions outside the declared scope remain visible |
