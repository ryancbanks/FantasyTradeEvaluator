...
class LeagueHistoryStoreTests(unittest.TestCase):
    ...
    def test_snapshot_ingestion_copies_only_bundle_bounded_league_evidence(self):
        ...
        later = capture(self.key, captured_at=NOW)
        self.store.ingest(
            later,
        ...
        unrelated_id = "engine_" + "3" * 64
        unrelated = replace(first, league_key=make_league_key("espn", "other"))
        self.store.ingest(
            unrelated,
            bundle=HistoryBundleBinding(
                unrelated.league_key,
                2026,
                unrelated_id,
                unrelated.captured_at,
            ),
        )
        bounded = self.store.snapshot_for_bundle(BUNDLE_1)
        destination = LeagueHistoryStore(
            Path(self.temporary.name) / "copied-history.sqlite3"
        )

        revision = destination.ingest_snapshot(bounded)
        destination.ingest_snapshot(bounded)

        self.assertEqual(revision, bounded.history_revision)
        self.assertEqual(destination.snapshot_for_bundle(BUNDLE_1), bounded)
        self.assertIsNone(destination.snapshot_for_bundle(BUNDLE_2))
        self.assertIsNone(destination.snapshot_for_bundle(unrelated_id))
