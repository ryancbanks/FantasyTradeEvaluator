...
class LocalAppServiceTests(unittest.TestCase):
    ...
    def test_catalog_labels_legacy_bundles_with_a_rescan_recovery(self):
        ...
        legacy["schema_version"] = ENGINE_BUNDLE_SCHEMA_VERSION - 1
        with TemporaryDirectory() as directory:
            service = LocalAppService(directory)
        ...
        self.assertEqual(
            catalog["bundles"][0]["schema_version"],
            ENGINE_BUNDLE_SCHEMA_VERSION - 1,
        )
    ...
    def test_catalog_reports_every_incompatible_saved_bundle_alongside_ready_data(self):
        ...
        future["schema_version"] = ENGINE_BUNDLE_SCHEMA_VERSION + 1
        with TemporaryDirectory() as directory:
            service = LocalAppService(directory)
    ...
    def test_concurrent_trade_timing_requests_share_one_calculation(self):
        ...
        def delayed_build(*args, **_kwargs):
    ...
    def test_dashboard_and_timing_share_the_same_scenario_baseline(self):
        bundle = engine_bundle()
        with TemporaryDirectory() as directory:
            service = LocalAppService(directory)
            service.import_bundle(bundle.to_record())
        ...
        build_baseline.assert_called_once()
    ...
    def test_not_ready_bundle_can_be_inspected_but_cannot_run_season_features(self):
        ...
        with TemporaryDirectory() as directory:
            ...
            league_catalog = service.league_bundle_catalog(profile["profile_id"])
        ...
        self.assertFalse(league_catalog["readiness"]["ready"])
        self.assertEqual(league_catalog["bundles"][0]["status"], "not_ready")
        self.assertEqual(
            league_catalog["bundles"][0]["data_readiness"]["status"],
            "not_ready",
        )
        self.assertIn(
    ...
    def test_unavailable_workspace_lookup_does_not_fall_back_or_block_core_views(self):
        bundle = engine_bundle()
        with TemporaryDirectory() as directory:
            service = LocalAppService(directory)
            service.import_bundle(bundle.to_record())
            with patch.object(
                service._league_workspaces,
                "data_directory_for_bundle",
                side_effect=RuntimeError("catalog unavailable"),
            ), patch.object(
                service,
                "_league_history_store",
                side_effect=AssertionError("must not fall back to another workspace"),
            ):
                insights = service.gm_insights(bundle.bundle_id)
                timing = service.trade_timing(bundle.bundle_id, "primary")
    ...
    def test_catalog_and_detail_never_materialize_the_full_player_outlook(self):
        ...
        self.assertEqual(
            catalog["data_readiness"]["status"],
            "ready_with_limitations",
        )
        self.assertNotIn("weeks", catalog["players"][0])
        self.assertEqual(detail["view"], "player_detail")
        self.assertEqual(detail["data_readiness"], catalog["data_readiness"])
    ...
    def test_listing_requires_a_rescan_for_a_prior_outer_bundle_schema(self):
        bundle = engine_bundle()
        ...
        with TemporaryDirectory() as directory:
            ...
            self.assertEqual(catalog[0]["status"], "legacy_requires_rescan")
            self.assertEqual(
                catalog[0]["schema_version"], ENGINE_BUNDLE_SCHEMA_VERSION - 1
            )
