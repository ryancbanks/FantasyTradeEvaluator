...
class LocalServerTests(unittest.TestCase):
    ...
    def test_static_assets_are_served_without_exposing_filesystem_paths(self):
        ...
        self.assertIn(
            b'addEventListener("change", () => changeBundle().catch(showError))',
            body,
        )
        self.assertIn(b'$("resultsPanel").classList.add("hidden")', body)
        self.assertIn(b'select.disabled = true', body)
        self.assertIn(b'$("bundleFile").disabled = running', body)
        self.assertIn(b'if (element.id === "cancelButton") continue', body)
        self.assertIn(b'job.operation?.progress?.determinate', body)
        self.assertIn(b'Boolean(activeCollectionClock)', body)
        self.assertIn(b'Boolean(activeCollection) || searchRunning', body)
        self.assertNotIn(b"pct.toFixed(0)", body)
        ...
        self.assertIn(b"refresh this page once", page)
        ...
        self.assertIn(b'id="leagueSelect"', page)
        self.assertIn(b'id="operationProgress"', page)
        self.assertIn(b'id="onlyMutualResults"', page)
        self.assertIn(b'id="minimumResultGain"', page)
        self.assertIn(b'id="resultSort"', page)
        self.assertLess(page.index(b'/progress_ui.js'), page.index(b'/app.js'))
        self.assertLess(page.index(b'/league_ui.js'), page.index(b'/app.js'))
        self.assertLess(page.index(b'/results_workbench.js'), page.index(b'/app.js'))
