...
class LeagueUiRuntimeTests(unittest.TestCase):
    ...
    def test_multiple_leagues_assignment_saved_team_archive_and_dark_ui(self):
        ...
        self.page.locator("#assignLeagueSelect").select_option(
            label="League One · 2026"
        )
        ...
        league_two_id = league_select.locator(
            "option", has_text="League Two · 2026"
        ).get_attribute("value")
        ...
        try:
            ...
            self.assertTrue(self.page.locator("#operationProgress").is_visible())
            self.assertEqual(
                self.page.locator("main").get_attribute("aria-busy"), "true"
            )
            self.assertTrue(
                self.page.locator("#operationProgressTrack").evaluate(
                    "element => element.classList.contains('indeterminate')"
                )
            )
            self.assertTrue(
        ...
        self.assertIsNone(self.page.evaluate("() => activeJob"))
        ...
        self.assertEqual(
            self.page.evaluate(
                """() => [
                  ...
                  document.querySelector('.league-workspace'),
                  document.querySelector('#leagueEditor'),
                  document.querySelector('.results-workbench')
        ...
        initial_result_count = self.page.locator("#resultsBody tr").count()
        self.assertGreater(initial_result_count, 0)
        self.page.locator("#minimumResultGain").fill("100")
        self.assertEqual(self.page.locator("#resultsBody tr").count(), 0)
        self.assertTrue(
            self.page.locator("#workbenchCount").text_content().startswith("0 of ")
        )
        self.page.locator("#minimumResultGain").fill("")
        self.assertEqual(
            self.page.locator("#resultsBody tr").count(), initial_result_count
        )

        with self.page.expect_download(timeout=15_000) as download_info:
            self.page.locator("#exportButton").click()
        self.assertTrue(download_info.value.suggested_filename.endswith(".xlsx"))
        self.page.wait_for_function(
    ...
    def test_empty_startup_then_profile_scoped_import_selects_ready_week(self):
        self.page.goto(
            self.server.app_url, wait_until="domcontentloaded", timeout=10_000
        )
        self.page.wait_for_function(
        ...
        league_select = self.page.locator("#leagueSelect")
        self.assertEqual(league_select.locator("option").count(), 1)
        self.assertEqual(
            league_select.locator("option").text_content(), "Add your first league"
        )
        self.assertTrue(league_select.is_disabled())
        self.assertEqual(
            self.page.locator("#bundleSelect option").text_content(),
            "No league selected",
        )
        self.assertTrue(self.page.locator("#startButton").is_disabled())
        self.assertTrue(self.page.locator("#collectButton").is_disabled())

        self.add_league("Imported League", "1239514350", "1530398")
        self.page.locator("#operationProgress").wait_for(state="hidden")
        profile_id = league_select.input_value()
        bundle = engine_bundle()
        self.page.locator("#bundleFile").set_input_files({
            "name": "weekly-bundle.json",
            "mimeType": "application/json",
            "buffer": json.dumps(bundle.to_record()).encode("utf-8"),
        })
        self.page.locator("#operationProgress").wait_for(state="hidden")
        self.assertFalse(
            self.page.locator("#errorBanner").is_visible(),
            self.page.locator("#errorBanner").text_content(),
        )
        self.page.wait_for_function(
            "bundleId => document.querySelector('#bundleSelect').value === bundleId",
        ...
        catalog = self.server.app_service.league_bundle_catalog(profile_id)
        self.assertEqual(
            [row["bundle_id"] for row in catalog["bundles"]],
            [bundle.bundle_id],
        )
        self.assertEqual(
            self.server.app_service.league_profiles()["unassigned_bundle_count"],
            0,
        )
        self.assertEqual(self.page_errors, [])
