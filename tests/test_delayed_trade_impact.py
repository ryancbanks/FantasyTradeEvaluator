...
class DelayedTradeImpactTests(unittest.TestCase):
    ...
    def test_prepared_inputs_are_reused_without_revalidating_or_redrawing(self):
        state, before, after, projections, eligibility, config = _inputs()
        baseline = prepare_season_baseline(
            state, before, projections, eligibility, config
        )
        ...
        with patch(
            "trade_snapshot.score_scenarios.PreparedScoreScenarios._team_week_score",
            side_effect=AssertionError("packed baseline scores must be replayed"),
        ):
            prepared = prepare_delayed_baseline(baseline)

        with patch(
            "trade_snapshot.score_scenarios._validated_projections",
            side_effect=AssertionError("validated projections must be reused"),
        ):
            delayed = prepared.roster_change(after, ("a", "b"))

        self.assertEqual(delayed.after_rosters, after)
    ...
    def test_conditioned_baseline_cache_is_lru_bounded(self):
        state, before, after, projections, eligibility, config = _inputs()
        baseline = prepare_season_baseline(
            state, before, projections, eligibility, config
        )
        ...
        with patch(
            "trade_snapshot.delayed_trade_impact._MAX_CONDITIONED_BEFORE_CACHE_SIZE",
            2,
        ):
            delayed.project_conditioned(2, (0,))
            delayed.project_conditioned(2, (1,))
            delayed.project_conditioned(2, (0,))
            delayed.project_conditioned(2, (2,))

        self.assertEqual(
            tuple(prepared._conditioned_before_cache),
            ((0,), (2,)),
        )
