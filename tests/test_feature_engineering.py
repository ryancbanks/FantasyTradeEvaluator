...
class StrengthFeatureEngineeringTests(unittest.TestCase):
    ...
    def test_unpublished_provider_is_excluded_from_remaining_projection_totals(self):
        ...
        changed = (projections[0], unpublished, *projections[2:])
        evidence = tuple(
            row
            for row in grid_horizon_evidence(changed)
            if not (
        ...
        self.assertEqual(p1["projection_yahoo_full_ros_available"], 0)
        self.assertEqual(p1["projection_yahoo_full_ros_points"], 0)
        self.assertEqual(p1["projection_ensemble_full_ros_points"], 32)
